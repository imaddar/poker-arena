...
func runRepositoryContractTests(t *testing.T, mkRepo func(t *testing.T) Repository) {
	t.Helper()

	t.Run("Contract_CreateAndListHandsOrderedByHandNo", func(t *testing.T) {
		repo := mkRepo(t)
		now := time.Now().UTC()
	...
	t.Run("Contract_AppendAndListActionsPreserveInsertionOrder", func(t *testing.T) {
		repo := mkRepo(t)
		ensureTableRunForContract(t, repo, "t1")
	...
	t.Run("Contract_CompleteHandUpdatesFinalState", func(t *testing.T) {
		repo := mkRepo(t)
	...
	t.Run("Contract_AppendActionRequiresExistingHand", func(t *testing.T) {
		repo := mkRepo(t)
	...
	t.Run("Contract_ConcurrentAppendAndReadIsSafe", func(t *testing.T) {
		repo := mkRepo(t)
		ensureTableRunForContract(t, repo, "table-1")
...
func ensureTableRunForContract(t *testing.T, repo Repository, tableID string) {
	t.Helper()
	err := repo.UpsertTableRun(TableRunRecord{
		TableID:        tableID,
		Status:         TableRunStatusRunning,
